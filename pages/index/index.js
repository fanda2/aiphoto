...
Page({
  ...
  onReachBottom: function () {
    var that = this;
    that.setData({
      basepage: that.data.basepage + 1
    })
    var id = that.data.requestId; //获得当前所处页面
    if (Math.ceil(that.data.postTotal / 10) >= that.data.basepage) {
      if (id == 1) {
        that.pst_recommend();
      } else if (id == 2) {
        that.pst_nearby();
